...
public class RoleServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2920048788955178121L;
	...
	public void list(Connection connection, HttpServletRequest request,
		...
		StringBuilder sql = new StringBuilder("select id, name, demo, create_time createTime, update_time updateTime from t_role");
		String sqlCount = "select count(*) as count from t_role";
...
//		}
		...
		// 查询DB
		List<Role> roles = roleDao.queryList(connection, sql.toString(), queryObjects.toArray());
		Long count = roleDao.queryForValue(connection, sqlCount, queryObjects.toArray());

		// 返回 JSON 数据
		ObjectMapper mapper = new ObjectMapper();
		System.out.println(roles);
		System.out.println(mapper.writeValueAsString(roles));
		String res = "{\"total\":" + count + ",\"rows\":" + mapper.writeValueAsString(roles) + "}";
	...
	public void update(Connection connection, HttpServletRequest request,
		...
		if (id == null || "".equals(id.trim()))
			throw new IllegalArgumentException("修改角色的ID不能为空或 Null");
		...
		boolean flg = false;
		try {
			roleDao.update(connection, sql.toString(), params.toArray());
			flg = true;
		} catch (Exception e) {
			throw e;
		} finally {
	...
	public void delete(Connection connection, HttpServletRequest request,
		...
		try {
			Integer num = roleDao.update(connection, sql, id);
	...
	public void save(Connection connection, HttpServletRequest request, HttpServletResponse response)
		...
		// 拼接添加的 SQL 语句
		String sql = "insert into t_role (name,demo,create_time,update_time) values (?,?,?,?)";
		RoleDao roleDao = new RoleDao();

		// 更新 DB
		boolean flg = false;
		try {
			roleDao.insert(connection, sql, name, demo, new Date(), new Date());
			flg = true;
		} catch (Exception e) {
			throw e;
		} finally {
	...
	public void checkName(Connection connection, HttpServletRequest request,
		...
		// 拼接 SQL
		String sql = "select count(*) as count from t_role where name = ?";
		
		// 查询 DB
		RoleDao roleDao = new RoleDao();
		Long count = roleDao.queryForValue(connection, sql, name);
